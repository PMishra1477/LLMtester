...
class MetaClient(BaseClient):
    """Client for Meta models (LLaMA)"""

    def __init__(self, model: str, config: Dict[str, Any]):
        super().__init__(model, config)
        self.api_url = config.get("api_url", "https://api.together.xyz/v1/completions")

    def execute(self, test_case: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """Execute a test with Meta models (via Together API or similar)"""
        try:
    ...
    def generate(self, prompt: str, context: Optional[str] = None, parameters: Optional[Dict[str, Any]] = None) -> str:
        """Generate text from Meta model - compatibility method for TestExecutor."""
        try:
